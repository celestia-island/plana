// This file was generated by [ts-rs](https://github.com/Aleph-Alpha/ts-rs). Do not edit this file manually.

export type AddDocumentRequest = { knowledge_base_id: string, content: string, title?: string, source_url?: string, metadata?: { [key in string]: string }, };

export type AddDocumentResponse = { document_id: string, success: boolean, error?: string, };

export type Agent = "HapLotes" | "SkoPeo" | "HubRis" | "KaLos" | "NeiKos" | "SkeMma" | "ApoRia" | "EleOs" | "EpieiKeia" | "OreXis" | "PhiLia" | "PoleMos" | "WebAutomation" | "ClassicSoftwareEngineering";

export type AgentBadge = string;

export type AgentErrorCode = "model_no_providers" | "model_no_models" | "model_tier_mismatch" | "model_all_excluded" | "model_env_incomplete" | "model_selection_retry_exhausted" | "llm_call_failed" | "llm_empty_response" | "llm_rate_limited" | "llm_auth_failed" | "llm_timeout" | "cosmos_no_connection" | "cosmos_tool_failed" | "cosmos_local_unavailable" | "chain_max_depth" | "chain_cycle" | "chain_failed" | "skill_failed" | "skill_empty_output" | "skill_missing_report";

export type AgentListResponseParams = { agents: Array<TuiAgentInfo>, };

export type AgentPatch = { agent_id: string, agent_number?: AgentBadge, agent_type?: Agent, version: bigint, llm_working_changed?: boolean, work_status?: WorkStatus, current_model?: string, model_tier?: ModelTier, llm_handle?: string, token_usage_delta?: [number, number], token_usage_absolute?: [number, number], request_state?: RequestState, mcp_tool_calls_delta?: number, skill_calls_delta?: number, cpu_usage?: number, memory_mb?: bigint, completion_outcome?: CompletionOutcome, retry_count?: number, max_retries?: number, current_stage?: string, next_stage?: string, current_tool_name?: string, parent_id?: string, };

export type AgentReportParams = { report_type: ReportType, agent_type: Agent, agent_id: string, agent_number?: string, title: string, content: string, summary?: string, timestamp: string, preset_options: Array<string>, model_name?: string, token_usage?: [number, number], skill_count?: number, mcp_count?: number, next_route?: RouteInfo, stream?: LlmStream, error?: StructuredAgentError, };

export type AgentResponseParams = { agent_type: Agent, agent_id: string, agent_number?: string, content: string, timestamp: string, parent_id?: string, };

export type AgentSnapshotData = { version: bigint, timestamp: bigint, agents: Array<TuiAgentInfo>, };

export type AgentSnapshotParams = { snapshot: AgentSnapshotData, };

export type AgentStatus = "Initializing" | "Online" | "Busy" | "Offline" | "Error";

export type AgentStreamingChunkParams = { agent_type: Agent, agent_id: string, agent_number?: string, chunk: string, is_done: boolean, timestamp: string, task_id?: string, chunk_kind?: StreamChunkKind, };

export type AgentTransferParams = { agent_type: Agent, agent_id: string, agent_number?: string, from_skill: string, to_skill: string, summary?: string, model_name?: string, token_usage?: [number, number], stream?: LlmStream, };

export type AgentUpdateParams = { agent: TuiAgentInfo, };

export type ApiKeyInfoResponseParams = { info: KeyInfo, };

export type ArbiterLockdownResponseParams = { ok: boolean, error?: string, };

export type ArbiterRestoreResponseParams = { ok: boolean, error?: string, };

export type ArbiterStatusData = { locked_down: boolean, active_policies: number, violations: Array<string>, mode: string, };

export type ArbiterStatusResponseParams = { ok: boolean, status?: ArbiterStatusData, error?: string, };

export type AskAnswerSource = "Human" | "Auto" | "Timeout";

export type AskHumanReplyParams = { consultation_id: string, selected_options: Array<string>, custom_answer?: string, answered_by: AskAnswerSource, timestamp: string, };

export type AskHumanRequestParams = { consultation_id: string, agent_type: Agent, agent_id: string, agent_number?: string, question: string, question_localized: string, context?: string, options: Array<string>, recommended?: string, timestamp: string, };

export type AuthChangePasswordResponseParams = { ok: boolean, error?: string, };

export type AuthDeleteUserResponseParams = { ok: boolean, error?: string, };

export type AuthGetUserResponseParams = { ok: boolean, user?: UserProfileSummary, error?: string, };

export type AuthListUsersResponseParams = { ok: boolean, users?: Array<UserProfileSummary>, error?: string, };

export type AuthLoginResponseParams = { ok: boolean, token?: string, session_id?: string, user_id?: string, username?: string, display_name?: string, role?: string, error?: string, };

export type AuthRegisterResponseParams = { ok: boolean, user_id?: string, username?: string, error?: string, };

export type AutoModeUpdateParams = { enabled: boolean, timeout_secs?: bigint, };

export type BaseAckParams = { message_id: string, };

export type BaseErrorParams = { code: string, message: string, };

export type BaseHeartbeatParams = { timestamp: bigint, };

export type CompletionOutcome = "None" | "Reported" | "Failed";

export type ConfiguredProviderInfo = { provider_name: string, display_name: string, api_endpoint?: string, default_model: string, is_enabled: boolean, };

export type ConfiguredProvidersListParams = { providers: Array<ConfiguredProviderInfo>, };

export type ContainerInfo = { id: string, name: string, status: ContainerStatus, cpu_usage: number, memory_mb: bigint, image: string, agent_type?: string, parent_id?: string, branch_level: number, is_cosmos: boolean, branch?: string, is_read_only: boolean, badge?: string, current_skill?: string, workspace_id?: string, workspace_path?: string, git_remote_url?: string, git_branch?: string, };

export type ContainerLogEntryParams = { instance_uuid: string, entry: LogEntryData, };

export type ContainerPatch = { container_id: string, version: bigint, status_changed?: ContainerStatus, cpu_usage_changed?: number, memory_usage_changed?: bigint, branch_changed?: string, is_read_only_changed?: boolean, badge_changed?: string, current_skill_changed?: string, };

export type ContainerSnapshotData = { version: bigint, timestamp: bigint, containers: Array<ContainerInfo>, };

export type ContainerSnapshotParams = { snapshot: ContainerSnapshotData, };

export type ContainerStatus = "created" | "running" | "paused" | "restarting" | "removing" | "exited" | "dead" | "unknown";

export type CosmosContainerInfo = { container_id: string, container_name: string, agent_type: string, instance_uuid: string, socket_path: string, image: string, branch?: string, badge?: string, };

export type CosmosOperationLogEntry = { timestamp: string, tool_name: string, params_preview: string, success: boolean, result_preview: string, error: Array<string>, };

export type CreateKnowledgeBaseRequest = { name: string, description?: string, embedding_model?: EmbeddingModel, custom_embedding_endpoint?: string, tags: Array<string>, };

export type CreateKnowledgeBaseResponse = { knowledge_base_id: string, success: boolean, error?: string, };

export type CreateSubscriptionRequest = { knowledge_base_id: string, subscription_type: SubscriptionType, url: string, sync_path?: string, sync_interval_hours?: bigint, name?: string, };

export type CreateSubscriptionResponse = { subscription_id: string, success: boolean, error?: string, };

export type CustomAgentInfo = { name: string, display_name: string, description: string, skills_count: number, source: string, version?: string, last_updated?: string, };

export type CustomAgentListResponseParams = { agents: Array<CustomAgentInfo>, };

export type DeleteKnowledgeBaseResponse = { success: boolean, error?: string, };

export type DeleteSubscriptionResponse = { success: boolean, error?: string, };

export type DisplayInfoParams = { id: number, name: string, width: number, height: number, };

export type DocumentStatus = "Pending" | "Indexing" | "Indexed" | "Error";

export type EmbeddingModel = "OpenAiSmall" | "OpenAiLarge" | "OpenAiAda" | "Custom";

export type EndpointValidatedParams = { provider_name: string, is_reachable: boolean, latency_ms?: bigint, error?: string, };

export type EntrypointApiConfigInfo = { protocol: string, base_url: string, chat_endpoint: string, models_endpoint?: string, auth_type: string, auth_header?: string, env_var: string, };

export type EntrypointConfigInfo = { id: string, name: { [key in string]: string }, type: string, billing_type: string, plan_tier: string, api: EntrypointApiConfigInfo, defaults: EntrypointDefaultsInfo, quotas: Array<QuotaInfo>, models: Array<string>, };

export type EntrypointDefaultsInfo = { deep: Array<string>, normal: Array<string>, basic: Array<string>, max_concurrent: MaxConcurrentInfo, };

export type GetKnowledgeBaseResponseParams = { knowledge_base: KnowledgeBaseInfo | null, };

export type GlobalSnapshotData = { version: bigint, timestamp: bigint, agents: Array<TuiAgentInfo>, containers: Array<ContainerInfo>, active_tasks: Array<TaskInfo>, };

export type GlobalSnapshotParams = { snapshot: GlobalSnapshotData, };

export type HandshakeAckParams = { ok: boolean, error?: string, };

export type HumanReviewRequestParams = { review_id: string, agent_type: Agent, agent_id: string, title: string, content: string, timestamp: string, };

export type HumanReviewResponseParams = { review_id: string, choice: string, comment: string, };

export type IceServerParams = { urls: Array<string>, username?: string, credential?: string, };

export type KbGenericResponseParams = { ok: boolean, error?: string, id?: string, };

export type KeyInfo = { provider: string, display_name: string, has_key: boolean, created_at: string, updated_at: string, source: string, };

export type KeysListResponseParams = { keys: Array<KeyInfo>, };

export type KnowledgeBaseFilters = { status?: KnowledgeBaseStatus, tags?: Array<string>, };

export type KnowledgeBaseInfo = { id: string, name: string, description?: string, status: KnowledgeBaseStatus, embedding_model?: EmbeddingModel, custom_embedding_endpoint?: string, document_count: number, subscription_count: number, tags: Array<string>, created_at: string, updated_at: string, };

export type KnowledgeBaseStatus = "Uninitialized" | "Indexing" | "Ready" | "Error";

export type Layer2AgentInfo = { name: string, description: string, mcp_count: number, skills_count: number, languages: Array<string>, enabled: boolean, };

export type Layer2AgentListResponseParams = { agents: Array<Layer2AgentInfo>, };

export type Layer2AgentMcpResponseParams = { agent_name: string, tools: Array<Layer2McpToolInfo>, };

export type Layer2AgentSkillsResponseParams = { agent_name: string, skills: Array<Layer2SkillInfo>, };

export type Layer2McpPromptResponseParams = { agent_name: string, tool: string, lang: string, content: string, name: string, };

export type Layer2McpToolInfo = { name: string, description: string, languages: Array<string>, references_layer1: Array<string>, references_layer2: Array<string>, related_items: Array<string>, referenced_by_items: Array<string>, };

export type Layer2SkillInfo = { name: string, description: string, languages: Array<string>, references_layer1: Array<string>, references_layer2: Array<string>, related_items: Array<string>, referenced_by_items: Array<string>, };

export type Layer2SkillPromptResponseParams = { agent_name: string, skill: string, lang: string, content: string, name: string, };

export type ListKnowledgeBasesResponseParams = { knowledge_bases: Array<KnowledgeBaseInfo>, };

export type LlmProviderConfiguredParams = { provider_name: string, success: boolean, error?: string, };

export type LlmStream = { segments: Array<StreamSegment>, };

export type LogEntryData = { source: string, instance_uuid?: string, level: string, target?: string, message: string, fields: Record<string, unknown>, created_at: string, };

export type MaxConcurrentInfo = { deep: number, normal: number, basic: number, };

export type McpToolResultParams = { tool_name: string, call_id: string, parameters_summary?: string, result: string, agent_type: Agent, agent_id: string, agent_number?: string, success: boolean, duration_ms?: bigint, };

export type ModelFsInfo = { id: string, name: string, provider_id: string, context_window: bigint, max_output_tokens: bigint, supports_vision: boolean, supports_function_calling: boolean, supports_streaming: boolean, supports_reasoning: boolean, tags: Array<string>, pricing?: ModelFsPricing, rate_multiplier?: number, rate_rules: Array<RateRuleInfo>, };

export type ModelFsInfoParams = { models: Array<ModelFsInfo>, };

export type ModelFsPricing = { input_per_million?: number, output_per_million?: number, cached_per_million?: number, };

export type ModelProviderConfigUpdatedParams = { provider_name: string, success: boolean, error?: string, };

export type ModelTier = "Deep" | "Normal" | "Basic";

export type ModelsSnapshotParams = { models: Array<SnapshotModelInfo>, };

export type OrchestrationStatusParams = { stage: SkillStage, agent: string, agent_type?: Agent, tool_name?: string, call_id?: string, parent_agent?: string, parameters_summary?: string, };

export type PeriodType = "Hour5" | "Day7" | "Month1";

export type PingParams = { timestamp: bigint, };

export type PolemosDeviceInfo = { node_id: string, name: string, address: string, status: string, workspace_path?: string, ide_id?: string, };

export type PolemosDeviceListParams = { devices: Array<PolemosDeviceInfo>, };

export type ProviderCapabilitiesInfo = { streaming: boolean, function_calling: boolean, vision: boolean, reasoning: boolean, };

export type ProviderDeletedParams = { provider_name: string, success: boolean, error?: string, };

export type ProviderEditedParams = { provider_name: string, success: boolean, error?: string, };

export type ProviderFsInfo = { id: string, name: { [key in string]: string }, protocol: string, category: string, base_url?: string, entry_points: Array<EntrypointConfigInfo>, models: Array<ModelFsInfo>, capabilities: ProviderCapabilitiesInfo, limits: ProviderLimitsInfo, pricing_model: string, };

export type ProviderFsInfoParams = { providers: Array<ProviderFsInfo>, };

export type ProviderLimitsInfo = { max_concurrent: number, rate_limit_per_minute?: number, timeout_seconds: bigint, };

export type ProviderRenamedParams = { provider_name: string, new_display_name: string, success: boolean, error?: string, };

export type ProvidersSnapshotParams = { providers: Array<SnapshotProviderInfo>, };

export type QueryKnowledgeBaseRequest = { knowledge_base_id?: string, query: string, top_k?: number, score_threshold?: number, };

export type QueryKnowledgeBaseResponse = { results: Array<QueryResultChunk>, success: boolean, error?: string, };

export type QueryResultChunk = { document_id: string, document_title?: string, content: string, score: number, source_url?: string, };

export type QuotaInfo = { data_limit: bigint, period_hours?: number, period_days?: number, billing_metric: string, };

export type RateRuleInfo = { timezone_offset: number, peak_start: number, peak_end: number, peak_multiplier: number, off_peak_multiplier: number, };

export type RegisterPolemosDeviceResponseParams = { success: boolean, error?: string, device?: PolemosDeviceInfo, };

export type ReportType = "query" | "human" | "reply" | "skill_terminal" | "skill_step" | "next_action_fallback" | "chain_max_depth" | "chain_cycle" | "skill_failed" | "skill_empty_output" | "skill_missing_report" | "error" | "system";

export type RequestState = "Idle" | "Waiting" | "Streaming" | "Retrying" | "WaitingTool";

export type RetryReason = "EmptyOutput" | "ReportNotCaptured" | { "LlmError": { message: string, } };

export type RouteInfo = { direction: string, target: string, target_token?: string, };

export type ScepterIdentityParams = { device_id: string, };

export type ServerLogEntryParams = { entry: LogEntryData, };

export type ServerVersionParams = { version: string, build_info: string, };

export type SkillStage = { "Started": string } | { "Done": string } | { "Complete": string } | { "Failed": string } | { "ToolCall": string } | { "Retrying": [string, number, number, RetryReason | null] } | { "TryingModel": [string, string] } | { "ModelFailed": [string, string, string] } | { "Nudging": string };

export type SnapshotModelInfo = { id: string, name: string, provider_name: string, model_type: string, context_length?: number, is_active: boolean, };

export type SnapshotProviderInfo = { name: string, display_name: string, api_endpoint: string, has_api_key: boolean, default_model?: string, is_active: boolean, };

export type StreamChunkKind = "Text" | "Thinking" | "DeepThinking";

export type StreamMcpEvent = { "McpCall": { tool_name: string, call_id: string, params_summary?: string, agent_type?: string, } } | { "McpResult": { tool_name: string, call_id: string, result: string, success: boolean, duration_ms?: bigint, agent_type?: string, } };

export type StreamSegment = { "Text": { text: string, message_id?: string, } } | { "Thinking": { text: string, message_id?: string, } } | { "DeepThinking": { text: string, message_id?: string, } } | { "McpCall": { tool_name: string, call_id: string, params: unknown, agent_type?: string, message_id?: string, } } | { "McpResult": { tool_name: string, call_id: string, success: boolean, data: unknown, duration_ms?: bigint, agent_type?: string, message_id?: string, } };

export type StreamingTailParams = { agent_id: string, tail: string, };

export type StructuredAgentError = { code: AgentErrorCode, detail?: string, context: { [key in string]: string }, };

export type SubscribeCustomAgentResponseParams = { success: boolean, error?: string, agent?: CustomAgentInfo, skills: Array<string>, permissions: Array<string>, };

export type SubscribeLogsResponseParams = { ok: boolean, error?: string, entries: Array<LogEntryData>, };

export type SubscriptionStatus = "NotSynced" | "Syncing" | "Synced" | "Error" | "Paused";

export type SubscriptionType = "GitHubRepo" | "GitRepo" | "Website" | "Rss" | "LocalDirectory";

export type SwitchWorkspaceResponseParams = { success: boolean, workspace_id: string, error?: string, };

export type SyncSubscriptionRequest = { subscription_id: string, force_full_sync?: boolean, };

export type SyncSubscriptionResponse = { success: boolean, error?: string, added_documents?: number, updated_documents?: number, deleted_documents?: number, };

export type SystemMessageParams = { notification: SystemNotification, timestamp: string, };

export type SystemNotification = "web_ui_started" | "web_ui_stopped" | "web_ui_restarted" | { "web_ui_error": { error: string, } } | { "web_ui_url": { url: string, } } | { "container_error": { container: string, error: string, } } | { "cosmos_error": { agent: string, error: string, } } | { "server_error": { error: string, } } | { "auto_mode_changed": { enabled: boolean, timeout_secs: bigint | null, } } | "auto_mode_usage" | { "workspace_opened": { repo_url: string, branch: string | null, } } | { "workspace_error": { error: string, } } | { "generic": { key: string, params: Array<string>, } } | { "security_policy_changed": { action: string, details: string, changed_by: string, } } | { "security_tool_blocked": { agent: string, tool: string, reason: string, } } | { "screen_session_started": { session_id: string, node_id: string, } } | { "screen_session_ended": { session_id: string, node_id: string, } };

export type TaskCreatedParams = { task_id: string, issue_id: string, title: string, description?: string, assigned_agent?: Agent, parent_task_id?: string, badge?: string, tags?: Array<string>, };

export type TaskInfo = { id: string, issue_id: string, title: string, status: TaskStatus, progress: number, assigned_agent?: string, };

export type TaskPatch = { task_id: string, version: bigint, status_changed?: TaskStatus, progress_changed?: number, };

export type TaskStatus = "not_started" | "in_progress" | "paused" | "completed" | "failed" | "warning" | { "waiting": { deadline: string, handle: string, } };

export type TaskStatusUpdateParams = { task_id: string, status: TaskStatus, progress: number, };

export type TasksSnapshotData = { version: bigint, timestamp: bigint, tasks: Array<TaskInfo>, };

export type TasksSnapshotParams = { snapshot: TasksSnapshotData, };

export type TuiAgentInfo = { agent_type: Agent, agent_number?: string, agent_uuid?: string, agent_id: string, status: AgentStatus, llm_working: boolean, cpu_usage: number, memory_mb: bigint, parent_id?: string, work_status?: WorkStatus, current_model?: string, model_tier?: ModelTier, llm_handle?: string, token_usage?: [number, number], mcp_tool_calls: number, request_state: RequestState, completion_outcome: CompletionOutcome, retry_count: number, max_retries: number, };

export type UnsubscribeCustomAgentResponseParams = { success: boolean, error?: string, };

export type UnsubscribeLogsResponseParams = { ok: boolean, error?: string, };

export type UsagePeriodData = { user_id: string, period_type: PeriodType, used_tokens: bigint, cost: number, start_time: string, remaining_tokens?: bigint, remaining_cost?: number, };

export type UsagePeriodResponseParams = { data: Array<UsagePeriodData>, };

export type UserConfigResponseParams = { config: UserInfoConfig, };

export type UserInfoConfig = { preferred_language: string, default_models: { [key in string]: string }, enabled_providers: Array<string>, enabled_models: Array<string>, model_priorities: { [key in string]: number }, auto_import_from_env: Array<string>, };

export type UserProfileSummary = { id: string, username: string, email: string, display_name: string, role: string, is_active: boolean, };

export type VmSnapshotParams = { agent_id: string, globals: unknown, container_info?: CosmosContainerInfo, tool_list: Array<string>, op_log: Array<CosmosOperationLogEntry>, };

export type WebUiControlResponseParams = { command: string, success: boolean, message: string, url?: string, };

export type WebUiStatusParams = { running: boolean, url?: string, container_id?: string, };

export type WebrtcAnswerParams = { session_id: string, node_id: string, sdp: string, displays: Array<DisplayInfoParams>, };

export type WebrtcIceCandidateParams = { session_id: string, node_id: string, candidate: unknown, };

export type WebrtcIceServersParams = { ice_servers: Array<IceServerParams>, };

export type WebrtcOfferParams = { session_id: string, node_id: string, sdp: string, };

export type WorkStatus = "Thinking" | "StreamingResponse" | { "Executing": { skill_name: string, } } | { "Retrying": { retry_count: number, max_retries: number, } } | "Nudging" | "Completed" | "RequestFailed" | "Failed" | "ToolLoopTerminated" | "CallingTool";

export type WorkspaceStatusParams = { workspace_id: string, display_name?: string, connection_kind: string, resolved_path?: string, remote_url?: string, branch?: string, host_id?: string, };

export type YoloCycleCompleteParams = { loop_count: bigint, duration_ms: bigint, };

export type YoloCycleStepParams = { skill: string, loop_count: bigint, status: string, token_usage?: [number, number], model_name?: string, };

export type YoloFullConfig = { tiers: Array<YoloTierConfig>, };

export type YoloStartResponseParams = { ok: boolean, error?: string, };

export type YoloStatusResponseParams = { active: boolean, loop_count: bigint, started_at?: string, current_cycle?: string, tiers: Array<YoloTierStatus>, };

export type YoloStopResponseParams = { ok: boolean, error?: string, };

export type YoloTaskResult = { success: boolean, duration_ms: bigint, completed_at: string, error?: string, };

export type YoloTaskStatus = { agent: string, skill: string, enabled: boolean, last_result?: YoloTaskResult, };

export type YoloTaskTier = "realtime" | "periodic" | "daily" | "strategic";

export type YoloTerminateResponseParams = { ok: boolean, error?: string, };

export type YoloTierConfig = { tier: YoloTaskTier, enabled: boolean, interval_secs: bigint, tasks: Array<YoloTierTaskConfig>, };

export type YoloTierStatus = { tier: YoloTaskTier, enabled: boolean, interval_secs: bigint, last_run_at?: string, next_run_at?: string, tasks: Array<YoloTaskStatus>, };

export type YoloTierTaskConfig = { agent: string, skill: string, enabled: boolean, };
