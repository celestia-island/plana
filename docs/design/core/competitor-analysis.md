...
- **Core loop** (~300 lines in `swarm/core.py`):
  ...
  3. Switch Agent if function returns an Agent
...
- **Core loop** (in `src/core/`):
  ...
  2. Analyze workspace (ASTs, regex search, file reads)
  3. Execute tools in a loop: file create/edit, terminal commands, browser actions
  4. Monitor outputs (linter errors, terminal output, browser screenshots)
  5. Auto-fix issues, iterate until task complete
...
- **Core loop** (`base_coder.py`):
  ...
  2. Send prompt + repo map + files to LLM
  3. Parse LLM response for edit instructions (unified diffs, search/replace blocks, whole file rewrites)
  4. Apply edits, lint, run tests, auto-fix failures
  5. Git commit with sensible messages
...
2. **Multi-agent communication**: Only Google ADK has a formal inter-agent protocol (A2A). Most frameworks use ad-hoc message passing. A standardized protocol (like A2A) is a gap.

3. **Memory architectures**: Most frameworks have basic short-term memory. Few have sophisticated hierarchical memory (working, short-term, long-term) with automated context management. MetaGPT's filtering and ADK's compaction are the best examples.

4. **Tool exposure management**: All frameworks expose all tools to the LLM per-call. No framework dynamically subsets tools based on context/state/security level. This is an architectural gap.

5. **Code execution**: Only ADK has production-grade sandboxed code execution. ChatDev has basic code executor. Cline/Aider rely on native environment. This is a critical security gap across the ecosystem.

6. **Evaluation**: ADK and Cline have formal evaluation frameworks. Others rely on ad-hoc testing or research benchmarks. Embedded evaluation is a differentiator.

7. **OpenAI Swarm's deprecation** into the OpenAI Agents SDK signals a market trend toward production-grade frameworks over educational experiments.

8. **LangGraph's durable execution** is uniquely powerful for long-running agents — most frameworks assume short-lived tasks.

9. **ChatDev 2.0's zero-code approach** targets a fundamentally different user persona (non-developers) than most frameworks. This is orthogonal to Entelecheia's developer-first design.

10. **Cline and Aider** are applications, not frameworks. They demonstrate the power of tight tool integration (IDE, git, terminal, browser) but can't be composed into larger agent systems.
