...
2. **Session persistence** — dump/restore on disk for crash recovery or session migration
3. **Container sync** — push state to cosmos containers via `cosmos_set_rag_context()`
...
2. **Fallback routing** — When a containerized cosmos call fails for an internal tool, the system falls back to the local cosmos runtime. For non-internal tools, the fallback goes to in-process execution instead. This ensures cosmos operations never fail silently in containerized mode.
...
2. Builds a `NamespaceConfig` with per-agent tool lists and metadata (sync/async, unwrap_data)
3. Generates namespace JS via `build_namespace_js(&config)` which:
