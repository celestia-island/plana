...
| **14 Агентов (12 L1 + 2 L2)** | Все 14 агентов компилируются с реализациями инструментов MCP. 147 инструментов MCP всего — **все реальные**. Ноль макросов `unimplemented!()` или `todo!()` в кодовой базе. | Классические инструменты SE помечены `maturity: Stub` в метаданных, но имеют реальные реализации (вызовы подпроцессов cargo clippy, eslint, pylint, go vet; метрики кода; рефакторинг extract-function). | 🟢 Активный |
...
| **SkeMma** | 1 | Выполнение скриптов и поведение, смежное с MCP | 11 | 0 | 124 теста | 🟢 Реальный |
...
| **Клиент/сервер OPC UA** | Требуется интеграция клиента/сервера OPC UA. PoleMos обнаруживает порт 4840, но реальная реализация клиента/сервера OPC UA отсутствует. | Реальный клиент OPC UA для чтения со сторонних SCADA-устройств; сервер OPC UA для предоставления показаний датчиков entelecheia промышленным SCADA (Ignition/WinCC/iFix). | P1 |
