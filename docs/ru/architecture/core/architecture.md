...
| Репозиторий | Область |
|------------|-------|
| **entelecheia** | Оркестрация Scepter, 14 агентов (12 L1 + 2 активных L2; 4 L2 запланированы), среда выполнения Cosmos/IEPL, 32 общих крейта |
...
| **14 Агентов (12 L1 + 2 L2)** | Все 14 агентов компилируются с реализациями инструментов MCP. 148 инструментов MCP всего — **147 реальных, 1 заглушка** (`opcua_browse`). Ноль макросов `unimplemented!()` или `todo!()` в кодовой базе. | Классические инструменты SE помечены `maturity: Stub` в метаданных, но имеют реальные реализации (вызовы подпроцессов cargo clippy, eslint, pylint, go vet; метрики кода; рефакторинг extract-function). | 🟢 Активный |
...
| **Другие дизайны Layer2** | Нет дополнительных доменных агентов помимо двух вышеуказанных; `res/prompts/domain_agents/` содержит документы config/skill только для реализованных агентов | `docs/plans/` никогда не создавался | 🔴 Отсутствует |
...
Рабочее пространство компилирует 12 агентов Layer1 (129 инструментов MCP) и 2 активных крейта Layer2 (Web Automation — 11 инструментов, Classic Software Engineering — 7 инструментов; всего 148 инструментов MCP). Все агенты используют макрос `agent_mcp_module!` для регистрации инструментов MCP. Макрос поддерживает `skill_routing` для агентов, которым требуется перехват перед диспетчеризацией (например, двойная диспетчеризация `SkillExecutor` в Skopeo).

**Статус реализации инструментов:** 147 из 148 инструментов имеют реальные реализации. Единственная настоящая заглушка — `skemma::opcua_browse` (только проверка TCP-соединения, самоидентифицируется как заглушка). Ноль макросов `unimplemented!()` или `todo!()` существует где-либо в кодовой базе. Ни один инструмент не возвращает тривиальный `Ok(())` без реальной логики.
...
| **SkeMma** | 1 | Выполнение скриптов и поведение, смежное с MCP | 11 | 1 (`opcua_browse`) | 124 теста | 🟢 Реальный |
...
| **PoleMos** | 1 | Устройства, периферия, SSH, возможности информации об оборудовании | 24 | 0 | 3 теста | 🟡 Низкое покрытие |
...
- **Layer2 сегодня**: `web_automation` (11 инструментов MCP) и `classic-software-engineering` (7 инструментов MCP) — активные крейты Layer2. `classic-software-engineering` предоставляет статический анализ, проверку кода, проверки качества, предложения рефакторинга, диагностику LSP, извлечение символов и рефакторинг LSP — реализован в `packages/domain_agents/classic_software_engineering/`. Система плагинов WASI (`plugin_host`) с двойной песочницей wasmtime + boa TS содержит эталонный плагин вебхуков GitHub; архитектура Trigger (`TriggerDispatcher` / `TriggerTopic` / `TriggerConfig`) диспетчеризует внешние события в цепочки навыков.
- **Другие дизайны Layer2**: нет дополнительных доменных агентов помимо двух вышеуказанных. `res/prompts/domain_agents/` содержит документацию config/skill/mcp только для реализованных агентов L2. Изначально запланированная директория `docs/plans/` так и не была создана.
...
- **SkeMma OPC UA — единственная настоящая заглушка**: Выполняет только проверку TCP-соединения. Самоидентифицируется в выводе лога. Требуется реальная интеграция крейта `opcua`.
...
| IB-09 | Реальная интеграция клиента OPC UA | ожидает | `skemma::opcua_browse` возвращает реальные данные устройства; крейт `opcua` подключён | **Единственная настоящая заглушка в системе** (1 из 148 инструментов) |
...
| Инструменты-заглушки | 0 | 1 (`opcua_browse`) |
...
| **Чтение/запись Modbus** | ✅ **Полностью реализованы.** `skemma::modbus_read` (FC 01/02/03/04 с ограничением безопасности регистров) и `skemma::modbus_write` (FC 05/06/15/16 с ограничением белого списка записи) оба функционируют. | — | ✓ |
| **Обнаружение S7comm** | ✅ **Реализовано.** `polemos::s7comm_discover` подключается к TCP:102, получает информацию о CPU, сканирует номера DB, исследует структуру DB. Использует `s7comm_probe` от evernight. | — | ✓ |
| **Обнаружение последовательных портов** | ✅ **Реализовано.** `polemos::serial_discover` перечисляет порты, исследует скорости передачи, сканирует ID станций Modbus. | — | ✓ |
...
| **Клиент/сервер OPC UA** | Единственная настоящая заглушка в системе: `skemma::opcua_browse` (только проверка TCP-соединения, самоидентифицируется как заглушка). PoleMos обнаруживает порт 4840. | Реальный клиент OPC UA для чтения со сторонних SCADA-устройств; сервер OPC UA для предоставления показаний датчиков entelecheia промышленным SCADA (Ignition/WinCC/iFix). | P1 |
