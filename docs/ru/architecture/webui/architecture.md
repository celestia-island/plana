...
2. core проверяет учётные данные по shittim_chest_db (argon2 для паролей)
3. core запрашивает у entelecheia права групп пользователя (или читает из TTL-кеша)
4. core выпускает JWT с `{ sub: user_id, groups: [...] }`
5. Все последующие запросы несут JWT → core проверяет → пересылает в scepter для прокси-маршрутов
6. scepter проверяет JWT (общий секрет через переменную окружения) и применяет права уровня группы
