...
2. **Контейнеры — неизменяемые единицы**: CLI создаёт/уничтожает контейнеры, никогда не модифицирует работающие
3. **Сетевая изоляция**: Порт PG не открыт на хосте, доступен только во внутренней сети Docker
4. **Пассивный опрос для проверок здоровья**: Не полагается на события Docker (ненадёжны); напрямую опрашивает результаты inspect
