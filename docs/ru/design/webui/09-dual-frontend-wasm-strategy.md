...
2. **Контракт API**: Обе версии используют один и тот же REST API и протокол WebSocket
3. **Визуальная согласованность**: Обе версии отображают один и тот же UI в одинаковых состояниях
4. **Прогрессивная замена**: Функции чата и администрирования arona могут мигрировать на WASM независимо
...
2. **WASM — это улучшение, а не замена**: Миграция не влияет на существующих пользователей — обе версии используют один и тот же API бэкенда.
3. **Бэкенд, независимый от фреймворка**: Бэкенд shittim_chest не знает о реализации фронтенда. Любой HTTP/WS-клиент может интегрироваться.
4. **Tairitsu — это зависимость, а не внутренняя разработка**: Начало миграции на WASM зависит от зрелости внешнего фреймворка Tairitsu.
