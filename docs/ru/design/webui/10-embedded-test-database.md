...
2. **Строгое приведение типов**: PGlite строже, чем стандартный PostgreSQL. Запросы типа `uuid_column = text_value` не сработают — всегда приводите типы явно.
3. **Нет конкурентных раннеров тестов**: Все асинхронные тесты, разделяющие один экземпляр PGlite, должны выполняться последовательно в одной функции `#[test]`.
4. **Зависание пула при сбросе**: `sqlx::PgPool::close()` может зависнуть на неопределённое время. Используйте `std::process::exit(0)` для завершения процесса теста.
