...
2. Наличия файла-индикатора `/run/entelecheia/yolo_active`.
...
2. Читает последние журналы чата entelecheia и агрегирует использование токенов на
   модель. С `--lookback-secs 0` (по умолчанию) используется только последний журнал.
3. Определяет режим YOLO (маркер в журнале чата или файл-индикатор).
4. Строит список соавторов (сначала полномочие `Entelecheia`, если YOLO, затем модели)
