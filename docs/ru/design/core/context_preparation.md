...
2. **Извлечение детерминировано по структуре** — одна и та же стратегия запроса всегда производит одну и ту же категорию ответа
3. **Множественные углы обеспечивают покрытие** — запросы H/I/J охватывают разные измерения (состояние задачи, контекст ошибок, обоснование решений)
4. **Старый контекст остаётся доступным** — он не отбрасывается, а *запрашивается по требованию* во время фазы подготовки
...
2. Извлечь текущие индикаторы прогресса (что сделано, что в процессе, что заблокировано)
3. Сгенерировать запрос, который спрашивает: *"Каково текущее состояние всех активных задач и каковы следующие шаги?"*
...
2. Определить решения, где альтернативы рассматривались и были отклонены
3. Сгенерировать запрос, который спрашивает: *"Какие решения были приняты, какие альтернативы были отклонены и почему?"*
...
2. Сопоставить, какие файлы зависят от каких, какие функции вызывают какие, какие концепции связаны
3. Сгенерировать запрос, который спрашивает: *"Каковы ключевые зависимости и отношения между обсуждаемыми сущностями?"*
...
2. Ввод/вывод текущего навыка захватывается в сегменте D (ссылки на код)
3. Оставшиеся шаги цепочки сохраняются в результате извлечения K (непрерывность задачи)
