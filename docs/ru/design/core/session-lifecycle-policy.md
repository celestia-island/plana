...
```
Родительский разговор (инициирован пользователем)
├── Дочерний разговор (задача #1) — parent_conversation_id -> родитель
├── Дочерний разговор (задача #2) — parent_conversation_id -> родитель
└── Дочерний разговор (задача #3) — parent_conversation_id -> родитель
```
