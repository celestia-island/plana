...
2. **Персистентности сессии** — дамп/восстановление на диск для восстановления после сбоя или миграции сессии
3. **Синхронизации контейнеров** — отправка состояния в контейнеры cosmos через `cosmos_set_rag_context()`
...
2. Строит `NamespaceConfig` со списками инструментов для каждого агента и метаданными (sync/async, unwrap_data)
3. Генерирует JS пространства имён через `build_namespace_js(&config)`, которая:
