...
2. Внесите изменения с чёткими, атомарными коммитами
3. Запустите `just lint && just test` перед пушем
4. Откройте PR против ветки `dev`
5. Убедитесь, что CI проходит (сборка Rust, сборка npm, линтинг)
