...
2. GitHub Webhook пересылает событие в Scepter
3. Навык `issue_triage` классифицирует его как **bug**, приоритет **high**
4. HubRis декомпозирует задачу: (a) воспроизвести утечку (b) найти причину (c) реализовать исправление
5. KaLos читает соответствующие исходные файлы, SkeMma запускает диагностические скрипты
6. Агент фиксирует исправление и комментирует решение в `@github#42`
