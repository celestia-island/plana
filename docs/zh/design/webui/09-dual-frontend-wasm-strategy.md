...
2. **API 契约**：两个版本使用相同的 REST API 和 WebSocket 协议
3. **视觉一致性**：两个版本在相同状态下渲染相同的 UI
4. **渐进式替换**：arona 的聊天和管理功能可独立迁移到 WASM
...
2. **WASM 是增强，不是替代**：迁移不影响现有用户——两个版本使用相同的后端 API。
3. **框架无关的后端**：shittim_chest 后端不感知前端实现。任何 HTTP/WS 客户端均可集成。
4. **Tairitsu 是外部依赖，非内部开发**：WASM 迁移的启动取决于外部 Tairitsu 框架的成熟度。
