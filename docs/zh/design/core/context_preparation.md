...
2. **提取在结构上是确定性的**——相同的查询策略始终产生相同类别的答案
3. **多角度确保覆盖**——H/I/J 查询覆盖不同维度（任务状态、错误上下文、决策理由）
4. **旧上下文仍然可访问**——它没有被丢弃，而是在准备阶段*按需查询*
...
2. 提取当前进度指示器（什么已完成、什么在进行中、什么被阻塞）
3. 生成询问的查询：*"所有活动任务的当前状态是什么，以及下一步是什么？"*
...
2. 识别考虑和拒绝替代方案的决策
3. 生成询问的查询：*"做出了哪些决策，拒绝了哪些替代方案，以及为什么？"*
...
2. 映射哪些文件依赖哪些、哪些函数调用哪些、哪些概念相关
3. 生成询问的查询：*"所讨论实体之间的关键依赖和关系是什么？"*
...
2. 当前技能的输入/输出在段 D（代码引用）中捕获
3. 链的剩余步骤在提取结果 K（任务连续性）中保留
