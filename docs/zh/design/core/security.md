...
2. 工具声明："我要求 `System` 权限"
3. 权限门控检查：`skill_level >= tool_requirement` 且 `skill 被显式授予此工具`
4. 若任一检查失败：调用被阻止、记录并报告给 OreXis 哨兵
...
2. **SWC** 将 TypeScript 转译为 JavaScript（语法验证）
3. **Boa 引擎**在沙箱化上下文中执行 JavaScript
4. ES 模块导入解析为 `__native_dispatch` 调用
5. 每次分发通过具有完整类型检查的 `McpRouter` 路由
...
2. 哈希链接到之前的事件以实现防篡改
3. 持久化到 PostgreSQL，具有可配置的保留期
4. 可通过 CLI 查询（`entelecheia-cli trace-chain <badge>`）
