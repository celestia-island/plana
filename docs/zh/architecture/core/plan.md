...
2. ✅ 向 `SnowflakeManager` 添加子代/后代查找：`children_of`、`children_of_badge`、`most_recent_child_of`、`deepest_descendant`（`parent_id` → 反向索引）。
3. ✅ 基于 `FuturesUnordered` 的 `next_targets` 并行执行：`dispatch_parallel_targets` 通过 `parallel_dispatch::fan_out`（由 `Semaphore` 限界）将协调器的独立**叶子**目标并发扇出。串行 `invoke_skill_with_retries` 路径中的两个全局单例阻塞，处理方式如下：
...
4. ✅ 在两个分支路径中强制执行 `MAX_BRANCH_DEPTH`（`COSMOS_MAX_BRANCH_DEPTH`，默认 4）；子代现在以 `source.branch_level + 1` 注册，而非硬编码的 `1`。
