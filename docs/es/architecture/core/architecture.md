...
| Repositorio | Alcance |
|------------|-------|
| **entelecheia** | Orquestación Scepter, 14 agentes (12 L1 + 2 L2 activos; 4 L2 planificados), runtime Cosmos/IEPL, 32 crates compartidas |
...
| **PoleMos** | 1 | Capacidades de dispositivo, edge, SSH, información de hardware | 24 | 0 | 3 pruebas | 🟡 Baja cobertura de pruebas |
...
- **Capa2 hoy**: `web_automation` (11 herramientas MCP) y `classic-software-engineering` (7 herramientas MCP) son las crates de Capa2 activas. `classic-software-engineering` proporciona análisis estático, revisión de código, verificaciones de calidad, sugerencias de refactorización, diagnósticos LSP, extracción de símbolos y refactorización LSP — implementado en `packages/domain_agents/classic_software_engineering/`. Un sistema de plugins WASI (`plugin_host`) con wasmtime + sandbox dual boa TS aloja un plugin de webhook de GitHub de referencia; una arquitectura Trigger (`TriggerDispatcher` / `TriggerTopic` / `TriggerConfig`) despacha eventos externos a cadenas de habilidades.
- **Otros diseños de Capa2**: no existen agentes especialistas de dominio adicionales más allá de los dos anteriores. `res/prompts/domain_agents/` contiene documentos de configuración/habilidad/mcp solo para los agentes L2 implementados. El directorio `docs/plans/` originalmente planificado nunca fue creado.
...
| **Lectura/escritura Modbus** | ✅ **Completamente implementadas.** `skemma::modbus_read` (FC 01/02/03/04 con protección de seguridad de registros) y `skemma::modbus_write` (FC 05/06/15/16 con protección de lista blanca de escritura) ambas funcionales. | — | ✓ |
