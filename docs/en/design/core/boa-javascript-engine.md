...
2. Be **safe and sandboxed** — LLM-generated code is untrusted by nature.
3. Support **ES module imports** for the tool dispatch mechanism (e.g., `import { file_read } from 'agent'`).
4. Have **minimal startup overhead** — containers are ephemeral and must be ready quickly.
5. Be **cross-platform** and easy to compile for the container target.
...
2. **"Invoke and use" simplicity.** Boa is designed as a library-first engine. It can be instantiated, configured with host functions, and executed in a few lines of Rust code. There is no separate process to manage, no IPC bridge to maintain, and no event loop complexity. The `JsReplHandle` in COSMOS creates a dedicated OS thread for the Boa runtime and communicates via standard Rust channels — a clean, composable architecture.
