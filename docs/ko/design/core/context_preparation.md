...
2. **추출은 구조적으로 결정적** — 동일한 쿼리 전략이 항상 동일한 답변 범주를 생성
3. **다중 각도가 커버리지 보장** — H/I/J 쿼리가 서로 다른 차원(작업 상태, 오류 컨텍스트, 결정 근거)을 커버
4. **과거 컨텍스트는 접근 가능한 상태로 유지** — 폐기되지 않고 준비 단계 동안 *필요 시 쿼리*됨
...
2. 현재 진행 지표 추출 (완료된 것, 진행 중인 것, 차단된 것)
3. 다음을 묻는 쿼리 생성: *"모든 활성 작업의 현재 상태와 다음 단계는 무엇인가?"*
...
2. 대안이 고려되고 거부된 결정 식별
3. 다음을 묻는 쿼리 생성: *"어떤 결정이 내려졌고, 어떤 대안이 거부되었으며, 그 이유는 무엇인가?"*
...
2. 어떤 파일이 어떤 것에 의존하는지, 어떤 함수가 어떤 것을 호출하는지, 어떤 개념이 관련되는지 매핑
3. 다음을 묻는 쿼리 생성: *"논의된 엔터티 간 핵심 의존성과 관계는 무엇인가?"*
...
2. 현재 스킬의 입력/출력이 세그먼트 D (코드 참조)에 포착됨
3. 체인의 남은 단계가 추출 결과 K (작업 연속성)에 보존됨
