...
2. **API 계약**: 두 버전 모두 동일한 REST API와 WebSocket 프로토콜을 사용한다
3. **시각적 일관성**: 두 버전 모두 동일한 상태에서 동일한 UI를 렌더링한다
4. **점진적 교체**: arona의 채팅 및 관리자 기능은 독립적으로 WASM으로 마이그레이션될 수 있다
...
2. **WASM은 대체가 아닌 향상이다**: 마이그레이션은 기존 사용자에게 영향을 주지 않으며, 두 버전 모두 동일한 백엔드 API를 사용한다.
3. **프레임워크에 구애받지 않는 백엔드**: shittim_chest 백엔드는 프론트엔드 구현을 인식하지 않는다. 모든 HTTP/WS 클라이언트가 통합될 수 있다.
4. **Tairitsu는 내부 개발이 아닌 의존성이다**: WASM 마이그레이션의 시작은 외부 Tairitsu 프레임워크의 성숙도에 달려 있다.
