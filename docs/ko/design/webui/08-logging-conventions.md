...
2. **모든 작업 시작에 로그가 있다**: `네트워크 생성 중...`, `마이그레이션 실행 중...`, `shittim_chest 빌드 중...` — 사용자는 CLI가 무엇을 하고 있는지 알 수 있다.
3. **모든 작업 완료에 확인이 있다**: `shittim-chest가 0.0.0.0:80에서 시작됨`, `모든 서비스가 시작됨`.
4. **조용히 성공하는 작업은 로깅되지 않는다**: `ensure_network`는 네트워크가 이미 존재하면 출력하지 않아 노이즈를 방지한다.
5. **컨테이너 로그는 Docker API를 통해 가져온다**: CLI 자체는 비즈니스 로그를 작성하지 않으며, 오케스트레이션 작업 로그만 기록한다.
