...
2. **컨테이너는 불변 단위이다**: CLI는 컨테이너를 생성/파기하며, 실행 중인 컨테이너를 수정하지 않는다
3. **네트워크 격리**: PG 포트는 호스트에 노출되지 않으며, Docker 내부 네트워크 내에서만 접근 가능하다
4. **헬스 체크를 위한 수동 폴링**: Docker 이벤트(신뢰할 수 없음)에 의존하지 않고, inspect 결과를 직접 폴링한다
