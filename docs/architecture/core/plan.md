...
2. ✅ Add child/descendant lookup to `SnowflakeManager`: `children_of`,
   `children_of_badge`, `most_recent_child_of`, `deepest_descendant`
   (`parent_id` → reverse index).
3. ✅ `FuturesUnordered`-based parallel execution of `next_targets`:
...
4. ✅ Enforce `MAX_BRANCH_DEPTH` (`COSMOS_MAX_BRANCH_DEPTH`, default 4) in both
