...
| `-32051` | 派发超时被取消 |
| `-32052` | 延迟操作 id 未知（从未签发，或已过窗口被淘汰） |
...
- **取回。** `ops.result {op_id}` → 结果对象
  ...
  的 id 应答 `-32052`；窗口已过的 id 应答 `-32053`（随后条目即被清除）。
...
- **结算通知。** 当发起连接仍然在线时，数据通道推送
  ...
  已结算该操作的 worker 卡住）。因此客户端始终以 `ops.result` 兜底。
...
（11 例：在远小于上游耗时的超时上限下立即应答、结算后与重连后的取回、
从第二条连接以及经 HTTP 传输取回、结构化 `-32052`/`-32053`、建议性
`ops.settled` 通知、过期清理、`-32054` 容量拒绝、`ops.cancel` 标志），
外加 `RpcClient::await_op` 自身的用例套件
