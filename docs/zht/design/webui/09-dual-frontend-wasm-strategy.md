...
2. **API 契約**：兩個版本使用相同的 REST API 和 WebSocket 協定
3. **視覺一致性**：兩個版本在相同狀態下渲染相同的 UI
4. **漸進式替換**：arona 的對話和管理功能可獨立遷移至 WASM
...
2. **WASM 是增強，非替代**：遷移不影響現有使用者——兩個版本使用相同的後端 API。
3. **後端與框架無關**：shittim_chest 後端不知道前端實作方式。任何 HTTP/WS 客戶端都可以整合。
4. **Tairitsu 是依賴，非內部開發**：WASM 遷移的開始取決於外部 Tairitsu 框架的成熟度。
