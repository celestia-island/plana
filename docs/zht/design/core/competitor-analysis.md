...
- **核心循環**（`swarm/core.py` 中約 300 行）：
  1. 從當前 Agent 獲取完成
  2. 執行工具呼叫，附加結果
  3. 如果函數傳回 Agent 則切換 Agent
  4. 更新上下文變數
  5. 重複直到無更多工具呼叫或達到 max_turns
...
- **核心循環**（在 `src/core/` 中）：
  1. 解析使用者任務（文字 + 圖像）
  2. 分析工作區（AST、正則搜尋、檔案讀取）
  3. 在循環中執行工具：檔案建立/編輯、終端命令、瀏覽器動作
  4. 監控輸出（Linter 錯誤、終端輸出、瀏覽器截圖）
  5. 自動修復問題，迭代直到任務完成
...
- **核心循環**（`base_coder.py`）：
  ...
  2. 將提示 + 倉庫地圖 + 檔案發送給 LLM
  3. 解析 LLM 回應中的編輯指令（unified diffs、搜尋/替換區塊、整個檔案重寫）
  4. 應用編輯、Lint、執行測試、自動修復失敗
  5. 使用合理的訊息進行 Git 提交
...
2. **多代理通訊**：只有 Google ADK 具有正式的代理間協定（A2A）。多數框架使用臨時的訊息傳遞。標準化協定（如 A2A）是一個差距。

3. **記憶架構**：多數框架具有基本的短期記憶。很少有具備自動上下文管理的複雜階層式記憶（工作、短期、長期）。MetaGPT 的過濾和 ADK 的壓縮是最佳範例。

4. **工具暴露管理**：所有框架在每次呼叫中都將所有工具暴露給 LLM。沒有框架根據上下文/狀態/安全等級動態子集化工具。這是一個架構差距。

5. **程式碼執行**：只有 ADK 具有生產級沙箱化程式碼執行。ChatDev 有基本的程式碼執行器。Cline/Aider 依賴原生環境。這是整個生態系統中的一個關鍵安全差距。

6. **評估**：ADK 和 Cline 具有正式的評估框架。其他依賴臨時測試或研究基準。嵌入式評估是一個差異化因素。

7. **OpenAI Swarm 的廢棄**為 OpenAI Agents SDK 標誌著一個市場趨勢，朝向生產級框架而非教育實驗。

8. **LangGraph 的持久執行**對於長期運行的代理具有獨特的能力 — 多數框架假設短命任務。

9. **ChatDev 2.0 的零程式碼方法**針對與多數框架根本不同的使用者角色（非開發人員）。這與 Entelecheia 的開發者優先設計是正交的。

10. **Cline 和 Aider** 是應用程式，而非框架。它們展示了緊密工具整合（IDE、git、終端、瀏覽器）的威力，但無法組合成更大的代理系統。
