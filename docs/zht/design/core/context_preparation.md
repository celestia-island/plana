...
2. **提取在結構上是確定性的** — 相同的查詢策略始終產生相同類別的答案
3. **多角度確保覆蓋** — H/I/J 查詢涵蓋不同維度（任務狀態、錯誤上下文、決策理由）
4. **舊上下文保持可存取** — 它沒有被丟棄，而是在準備階段*按需查詢*
...
2. 提取當前進度指標（已完成什麼、進行中什麼、受阻什麼）
3. 生成一個查詢，詢問：*「所有活躍任務的當前狀態是什麼，以及下一步是什麼？」*
...
2. 識別曾考慮和拒絕替代方案的決策
3. 生成一個查詢，詢問：*「做出了哪些決策，拒絕了哪些替代方案，以及為什麼？」*
...
2. 映射哪些檔案依賴哪些、哪些函數呼叫哪些、哪些概念相關
3. 生成一個查詢，詢問：*「所討論的實體之間有哪些關鍵依賴關係和關聯？」*
...
2. 當前技能的輸入/輸出被捕獲在區段 D（程式碼參考）中
3. 鏈的剩餘步驟被保留在提取結果 K（任務連續性）中
