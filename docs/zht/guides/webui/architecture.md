...
2. shittim_chest 驗證 JWT，開啟到 scepter 的連線並轉發 JWT
3. 瀏覽器和 scepter 之間的雙向訊息轉發
4. shittim_chest 強制執行速率限制、記錄使用情況、管理連線生命週期
...
2. 透過 Let's Encrypt（Caddy 內建的 ACME）自動佈建 TLS 憑證
3. 將所有請求代理到 Docker 網路上的核心後端
