...
| Dépôt | Périmètre |
|------------|-------|
| **entelecheia** | Orchestration Scepter, 14 agents (12 L1 + 2 L2 actifs ; 4 L2 prévus), runtime Cosmos/IEPL, 32 crates partagées |
...
| **PoleMos** | 1 | Capacités périphérique, edge, SSH, info matériel | 24 | 0 | 3 tests | 🟡 Faible couverture test |
...
- **Layer2 aujourd'hui** : `web_automation` (11 outils MCP) et `classic-software-engineering` (7 outils MCP) sont les crates Layer2 actives. `classic-software-engineering` fournit analyse statique, revue de code, vérifications qualité, suggestions de refactoring, diagnostics LSP, extraction de symboles et refactoring LSP — implémenté dans `packages/domain_agents/classic_software_engineering/`. Un système de plugins WASI (`plugin_host`) avec double bac à sable wasmtime + boa TS héberge un plugin webhook GitHub de référence ; une architecture Trigger (`TriggerDispatcher` / `TriggerTopic` / `TriggerConfig`) distribue les événements externes aux chaînes de compétences.
- **Autres conceptions Layer2** : aucun agent spécialiste de domaine supplémentaire n'existe au-delà des deux ci-dessus. `res/prompts/domain_agents/` contient la documentation config/skill/mcp pour les agents L2 implémentés uniquement. Le répertoire `docs/plans/` initialement prévu n'a jamais été créé.
...
```
ACTUEL — allumé, pilote tiers dans la boucle :
  Plateforme agent externe (opencode / Claude Code / Cursor)
   │  lit ARCHITECTURE.md, décompose, distribue    ← nous ne fournissons PAS cela
   ▼
  Entelecheia : hooks chirurgie + chaînes compétences + conteneurs ← nous fournissons cela
   │  PreSurgeryCheckpoint → plan_execute → PostSurgeryRollback → merge
   ▼
  Base de code modifiée, validée (cargo check), commitée

CIBLE — auto-amorçage pur (PAS ENCORE) :
  Session Yolo (Hubris niveau supérieur, pas de pilote externe)
   ├── file_read("ARCHITECTURE.md") → analyser Backlog Itération
   ├── task_decompose → workplan_generate (planification seulement, pas I/O)
   ├── IEPL exec → distribuer chaîne compétences sous-agent
   └── PostSurgeryRollback → NoaMergeCommit → statut backlog auto-mise à jour
```
...
```
Hubris → Skopeo → Kalos/IEPL → Skemma → EvernightModbusAdapter → evernight → aoba → Modbus → vanne
                                                                                        ✅ écriture ok
```
...
```
Capteur réservoir H2 → lecture Modbus → evernight SensorPoller → TriggerDispatcher → plan Hubris → Skopeo → OreXis → confirmation humaine → écriture
         ❌ pas de boucle sondage            ❌ dans evernight 8B   ✅ sujets modbus    ❌ initiation plan Hubris de bout en bout non vérifiée
```
...
| **Lecture/écriture Modbus** | ✅ **Entièrement implémenté.** `skemma::modbus_read` (FC 01/02/03/04 avec verrouillage sécurité registres) et `skemma::modbus_write` (FC 05/06/15/16 avec verrouillage liste blanche écriture) tous deux fonctionnels. | — | ✓ |
...
```
┌─────────────────────────────────────────────────────────────────┐
│  SCADA Industriel (Ignition / WinCC / iFix)                      │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐        │
│  │ Client   │  │ Maître   │  │ Graphique│  │ Panneau  │        │
│  │ OPC UA   │  │ Modbus   │  │ Tendance │  │ Alarmes  │        │
│  └────┬─────┘  └────┬─────┘  └────┬─────┘  └────┬─────┘        │
│       │              │             │             │               │
├───────┼──────────────┼─────────────┼─────────────┼───────────────┤
│       │              │             │             │               │
│  ┌────▼──────────────▼─────────────▼─────────────▼──────────┐   │
│  │  evernight — Courtier Capacité Matériel                   │   │
│  │  ┌───────────┐ ┌──────────────┐ ┌────────────────────┐   │   │
│  │  │SensorPoll │ │ MaîtreModbus │ │ Client/srv OPC UA  │   │   │
│  │  │ (boucle)  │ │ (R/W coils)  │ │ (parcourir/r/w/sub)│   │   │
│  │  └─────┬─────┘ └──────┬───────┘ └────────┬───────────┘   │   │
│  │        │               │                  │               │   │
│  │  ┌─────▼───────────────▼──────────────────▼───────────┐   │   │
...
│                              │                                   │
│  ┌───────────────────────────▼────────────────────────────────┐  │
│  │  entelecheia — Orchestration Agent                         │  │
│  │  ┌─────────────┐  ┌──────────┐  ┌──────────────────────┐  │  │
│  │  │ TriggerDispatch│→│ Hubris   │→│ Skopeo → SkillChain │  │  │
│  │  │ (sujets hw)   │  │ (plan)   │  │ (exécuter)           │  │  │
│  │  └──────────────┘  └──────────┘  └──────────┬───────────┘  │  │
│  │  ┌─────────────┐  ┌──────────┐               │              │  │
│  │  │ OreXis       │  │ EpieiKeia│               │              │  │
│  │  │ (alarme +    │  │ (confir. │               │              │  │
...
│  │                         │                                    │  │
│  │  ┌──────────────────────▼─────────────────────────────────┐ │  │
│  │  │ Stockage partagé : TimeSeriesAdapter + journal agent noa│ │  │
│  │  └────────────────────────────────────────────────────────┘ │  │
│  └─────────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────┘
```
