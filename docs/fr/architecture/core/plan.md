...
2. ✅ Ajouter la recherche enfant/descendant à `SnowflakeManager` : `children_of`, `children_of_badge`, `most_recent_child_of`, `deepest_descendant` (`parent_id` → index inverse).
3. ✅ Exécution parallèle basée sur `FuturesUnordered` des `next_targets` : `dispatch_parallel_targets` distribue les cibles **feuilles** indépendantes d'un coordinateur de manière concurrente via `parallel_dispatch::fan_out` (limité par un `Semaphore`). Les deux bloqueurs singleton globaux dans le chemin série `invoke_skill_with_retries` sont gérés comme suit :
...
4. ✅ Appliquer `MAX_BRANCH_DEPTH` (`COSMOS_MAX_BRANCH_DEPTH`, par défaut 4) dans les deux chemins de fork ; les enfants s'enregistrent maintenant à `source.branch_level + 1` au lieu d'un `1` codé en dur.
