...
2. La présence du fichier sentinelle `/run/entelecheia/yolo_active`.
...
2. Lit le(s) journal(s) de chat entelecheia le(s) plus récent(s) et agrège l'utilisation de tokens par
   modèle. Avec `--lookback-secs 0` (par défaut), seul le journal le plus récent est utilisé.
3. Détecte le mode YOLO (marqueur de journal de chat ou fichier sentinelle).
4. Construit la liste des co-auteurs (autorité `Entelecheia` en premier si YOLO, puis modèles)
