...
- **Boucle centrale** (~300 lignes dans `swarm/core.py`) :
  ...
  2. Exécuter les appels d'outils, ajouter les résultats
  3. Changer d'Agent si une fonction retourne un Agent
  4. Mettre à jour les variables de contexte
  5. Répéter jusqu'à ce qu'il n'y ait plus d'appels d'outils ou que max_turns soit atteint
...
- **Boucle centrale** (dans `src/core/`) :
  ...
  2. Analyser l'espace de travail (ASTs, recherche regex, lectures de fichiers)
  3. Exécuter des outils en boucle : création/édition de fichiers, commandes terminal, actions navigateur
  4. Surveiller les sorties (erreurs linter, sortie terminal, captures d'écran navigateur)
  5. Auto-corriger les problèmes, itérer jusqu'à ce que la tâche soit terminée
...
- **Boucle centrale** (`base_coder.py`) :
  ...
  2. Envoyer prompt + carte du dépôt + fichiers au LLM
  3. Analyser la réponse LLM pour les instructions d'édition (diffs unifiés, blocs search/replace, réécritures de fichiers entiers)
  4. Appliquer les modifications, linter, exécuter les tests, auto-corriger les échecs
  5. Commit git avec des messages sensés
...
2. **Communication multi-agent** : Seul Google ADK a un protocole inter-agent formel (A2A). La plupart des frameworks utilisent un passage de messages ad-hoc. Un protocole standardisé (comme A2A) est une lacune.

3. **Architectures de mémoire** : La plupart des frameworks ont une mémoire à court terme basique. Peu ont une mémoire hiérarchique sophistiquée (travail, court terme, long terme) avec gestion automatique du contexte. Le filtrage de MetaGPT et le compactage d'ADK sont les meilleurs exemples.

4. **Gestion de l'exposition des outils** : Tous les frameworks exposent tous les outils au LLM par appel. Aucun framework ne sous-ensemble dynamiquement les outils en fonction du contexte/état/niveau de sécurité. C'est une lacune architecturale.

5. **Exécution de code** : Seul ADK a une exécution de code isolée de qualité production. ChatDev a un exécuteur de code basique. Cline/Aider dépendent de l'environnement natif. C'est une lacune de sécurité critique dans tout l'écosystème.

6. **Évaluation** : ADK et Cline ont des frameworks d'évaluation formels. Les autres dépendent de tests ad-hoc ou de benchmarks de recherche. L'évaluation intégrée est un différenciateur.

7. **La dépréciation d'OpenAI Swarm** vers l'OpenAI Agents SDK signale une tendance du marché vers des frameworks de qualité production plutôt que des expériences éducatives.

8. **L'exécution durable de LangGraph** est particulièrement puissante pour les agents de longue durée — la plupart des frameworks supposent des tâches de courte durée.

9. **L'approche zéro code de ChatDev 2.0** cible un persona utilisateur fondamentalement différent (non-développeurs) de la plupart des frameworks. C'est orthogonal à la conception orientée développeur d'Entelecheia.

10. **Cline et Aider** sont des applications, pas des frameworks. Ils démontrent la puissance d'une intégration d'outils étroite (IDE, git, terminal, navigateur) mais ne peuvent pas être composés en systèmes d'agents plus larges.
