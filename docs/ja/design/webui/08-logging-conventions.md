...
2. **すべての操作開始にログがある**: `ネットワークを作成中...`、`マイグレーションを実行中...`、`shittim_chestをビルド中...` — ユーザーはCLIが何をしているかを把握できます。
3. **すべての操作完了に確認がある**: `shittim-chest が 0.0.0.0:80 で起動しました`、`すべてのサービスが起動しました`。
4. **成功した操作はログに残さない**: ノイズを避けるため、ネットワークが既に存在する場合`ensure_network`は出力しません。
5. **コンテナログはDocker API経由で取得**: CLI自体はビジネスログを書き込まず、オーケストレーション操作ログのみを出力します。
