...
2. **疎結合統合**: entelecheia統合はオプションのプロキシレイヤーです。ユーザーはLLMチャットのみを使用するか、entelecheia経由でエージェントオーケストレーションを有効にするかを選択できます。
3. **段階的WASM**: Vue 3フロントエンドは「生きた仕様」として最初に提供され、WASM移行には明確な判断基準（フレームワークの成熟度、エコシステムのカバレッジ、開発リソース）があります。
4. **Dockerネイティブ**: すべてのサーバーサイドコンポーネントはbollard Docker APIを介して管理され、docker-composeに依存しません。
