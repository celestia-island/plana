...
2. **API契約**: 両バージョンは同じREST APIとWebSocketプロトコルを使用します
3. **視覚的一貫性**: 両バージョンは同一の状態で同一のUIをレンダリングします
4. **段階的置き換え**: aronaのチャットと管理機能は独立してWASMに移行できます
...
2. **WASMは強化であり置き換えではない**: 移行は既存のユーザーに影響しません — 両バージョンは同じバックエンドAPIを使用します。
3. **フレームワークに依存しないバックエンド**: shittim_chestバックエンドはフロントエンドの実装を意識しません。任意のHTTP/WSクライアントが統合できます。
4. **Tairitsuは依存関係であり内部開発ではない**: WASM移行の開始は外部のTairitsuフレームワークの成熟度に依存します。
