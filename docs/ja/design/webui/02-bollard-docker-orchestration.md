...
2. **コンテナは不変の単位**: CLIはコンテナを作成/破棄し、実行中のコンテナを変更することはありません
3. **ネットワーク分離**: PGポートはホストに公開されず、内部Dockerネットワーク内でのみ到達可能です
4. **ヘルスチェックのパッシブポーリング**: Dockerイベント（信頼性低）に依存せず、inspect結果を直接ポーリングします
