...
```text
exec({ code: "import { host_command_exec } from 'polemos'; const r = await host_command_exec({ command: 'cd /opt/entelecheia && rg \"\\\\.unwrap\\\\(\\\\)|\\\\.expect\\\\(|panic!|todo!\" --type rust -c 2>/dev/null | grep -v \"/tests/\" | sort -t: -k2 -rn | head -20', timeout: 30 }); console.log(r.data.stdout || r.data.stderr);" })
```
...
```json
exec({ code: "import { host_command_exec } from 'polemos'; const r = await host_command_exec({ command: 'cd /opt/entelecheia && find packages -name \"*.rs\" -exec wc -l {} + 2>/dev/null | sort -rn | head -15', timeout: 30 }); console.log(r.data.stdout || r.data.stderr);" })
```
