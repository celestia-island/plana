...
```json
exec({ code: "import { host_command_exec } from 'polemos'; const r = await host_command_exec({ command: 'cd /mnt/sdb1/entelecheia && find packages -name \"*.rs\" -exec wc -l {} + 2>/dev/null | sort -rn | head -20', timeout: 30 }); console.log(r.data.stdout || r.data.stderr);" })
```
...
```text
exec({ code: "import { host_command_exec } from 'polemos'; const r = await host_command_exec({ command: 'cd /mnt/sdb1/entelecheia && rg \"\\\\.unwrap\\\\(\\\\)|\\\\.expect\\\\(|panic!|todo!\" --type rust -c 2>/dev/null | sort -t: -k2 -rn | head -20', timeout: 30 }); console.log(r.data.stdout || r.data.stderr);" })
```
...
```json
exec({ code: "import { host_command_exec } from 'polemos'; const r = await host_command_exec({ command: 'cd /mnt/sdb1/entelecheia && cargo clippy -- -W clippy::all 2>&1 | grep -E \"dead_code|unused_imports|unused_variables\" | sort -u | head -30', timeout: 180 }); console.log(r.data.stdout || r.data.stderr);" })
```
...
```json
exec({ code: "import { host_command_exec } from 'polemos'; const r = await host_command_exec({ command: 'cd /mnt/sdb1/entelecheia && cargo clippy -- -W clippy::all 2>&1 | grep \"patch.*was not used\" | head -10', timeout: 180 }); console.log(r.data.stdout || r.data.stderr);" })
```
