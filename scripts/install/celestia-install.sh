...
Options:
  ...
  --skip-deploy        Skip remote deployment (build and install locally only)
  ...
  --target-pass        SSH password for target node (default: hydroSinap2024)
  --offline DIR        Path to pre-downloaded offline bundle directory.
                          Uses local rootfs + images, skips all network.
                          The directory must contain:
                            celestia-images-*.tar.gz
                          Also checks for pre-built binaries:
                            entelecheia, evernight, scriptum, shittim-cli
                          Generated by arona/scripts/rootfs/build-images.sh.
...
read_opencode_api_key() {
    c_step "Reading API key from opencode config"
    local key=""
    local config_paths=(
    ...
    for cfg in "${config_paths[@]}"; do
        if [[ -f "$cfg" ]]; then
            key=$(grep -oP '"API_KEY"\s*:\s*"[^"]*"|"Z_AI_API_KEY"\s*:\s*"[^"]*"|"Authorization"\s*:\s*"Bearer\s+\K[^"]+' "$cfg" 2>/dev/null | head -1 || true)
            if [[ -n "$key" ]]; then
                c_ok "Found API key in: $cfg"
                echo "$key"
                return 0
            fi
        fi
    done
    c_warn "No API key found in opencode config"
    return 1
}

ssh_exec() {
    sshpass -p "$SSH_PASS" ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 "${DEPLOY_USER}@${TARGET_IP}" "$@"
}
...
check_ssh_connectivity() {
    c_info "Checking SSH connectivity to ${DEPLOY_USER}@${TARGET_IP}..."
    if sshpass -p "$SSH_PASS" ssh -o StrictHostKeyChecking=no -o ConnectTimeout=5 \
        "${DEPLOY_USER}@${TARGET_IP}" "hostname" 2>/dev/null; then
        c_ok "SSH connectivity OK"
        return 0
    fi
    c_err "Cannot SSH to ${DEPLOY_USER}@${TARGET_IP}"
    return 1
}

ensure_sshpass() {
    if ! command -v sshpass >/dev/null 2>&1; then
        c_info "Installing sshpass..."
        run_root apt-get install -y -qq sshpass 2>/dev/null || \
        run_root dnf install -y sshpass 2>/dev/null || \
        { c_err "Cannot install sshpass. Install manually."; return 1; }
        c_ok "sshpass installed"
    fi
}
...
pull_docker_images() {
    ...
    if [[ -n "$OFFLINE_DIR" && -d "$OFFLINE_DIR" ]]; then
        local images_tarball
        images_tarball=$(ls -t "$OFFLINE_DIR"/celestia-images-*.tar.gz 2>/dev/null | head -1)
        if [[ -n "$images_tarball" && -f "$images_tarball" ]]; then
            c_info "Loading pre-pulled images from offline bundle: $(basename "$images_tarball")"
            if docker load -i "$images_tarball" 2>&1; then
                c_ok "Offline images loaded successfully"
                return 0
            fi
            c_warn "Image load failed — falling back to network pull."
        fi
    fi
...
copy_binaries_from_offline() {
    c_step "Checking offline directory for pre-built binaries"
    if [[ -z "$OFFLINE_DIR" || ! -d "$OFFLINE_DIR" ]]; then
        return 1
    fi
    local found=0
    local bins=("entelecheia" "evernight" "scriptum" "chest-cli")
    for bin in "${bins[@]}"; do
        local src="$OFFLINE_DIR/$bin"
        if [[ -f "$src" ]]; then
            local dest="$INSTALL_DIR/$bin"
            mkdir -p "$INSTALL_DIR"
            cp -f "$src" "$dest"
            chmod +x "$dest"
            c_ok "Copied $bin from offline bundle"
            found=1
        fi
    done
    if [[ "$found" -eq 1 ]]; then
...
deploy_to_target() {
    c_step "Deploying to target node: ${DEPLOY_USER}@${TARGET_IP}"
    ensure_sshpass || return 1
    check_ssh_connectivity || return 1

    local remote_install="/tmp/entelecheia-install"
    ssh_exec "mkdir -p ${remote_install} ${INSTALL_DIR} ${LOG_DIR}"

    local ente_bin="$SOURCE_ROOT/entelecheia/target/release/scepter"
    local ever_bin="$SOURCE_ROOT/evernight/target/release/evernight"
    local scr_bin="$SOURCE_ROOT/scriptum/target/release/scriptum"
    local shittim_bin="$INSTALL_DIR/chest-cli"

    if [[ -f "$ente_bin" ]]; then
        c_info "Copying entelecheia scepter..."
        scp_to_target "$ente_bin" "${DEPLOY_USER}@${TARGET_IP}:${remote_install}/scepter"
        ssh_exec "cp -f ${remote_install}/scepter ${INSTALL_DIR}/scepter && chmod +x ${INSTALL_DIR}/scepter"
        c_ok "scepter deployed"
    elif [[ -f "$INSTALL_DIR/entelecheia" ]]; then
        scp_to_target "$INSTALL_DIR/entelecheia" "${DEPLOY_USER}@${TARGET_IP}:${INSTALL_DIR}/scepter"
        ssh_exec "chmod +x ${INSTALL_DIR}/scepter"
        c_ok "scepter deployed (offline)"
    else
        c_warn "scepter binary not found — skipping"
    fi

    if [[ -f "$ever_bin" ]]; then
        c_info "Copying evernight..."
        scp_to_target "$ever_bin" "${DEPLOY_USER}@${TARGET_IP}:${INSTALL_DIR}/evernight"
        ssh_exec "chmod +x ${INSTALL_DIR}/evernight"
        c_ok "evernight deployed"
    fi

    if [[ -f "$scr_bin" ]]; then
        c_info "Copying scriptum..."
        scp_to_target "$scr_bin" "${DEPLOY_USER}@${TARGET_IP}:${INSTALL_DIR}/scriptum"
        ssh_exec "chmod +x ${INSTALL_DIR}/scriptum"
        c_ok "scriptum deployed"
    fi

    if [[ -f "$shittim_bin" ]]; then
        c_info "Copying shittim-chest..."
        scp_to_target "$shittim_bin" "${DEPLOY_USER}@${TARGET_IP}:${INSTALL_DIR}/chest-cli"
        ssh_exec "chmod +x ${INSTALL_DIR}/chest-cli"
        c_ok "shittim-chest deployed"
    fi

    c_info "Setting up scepter environment on target node..."
    local api_key
    api_key=$(read_opencode_api_key 2>/dev/null || echo "")
    if [[ -z "$api_key" ]]; then
        api_key="sk-your-key-here"
    fi
    local target_env="/tmp/entelecheia.env"
...
ENVEOF
    scp_to_target "$target_env" "${DEPLOY_USER}@${TARGET_IP}:${INSTALL_DIR}/.env"
    rm -f "$target_env"
    c_ok "Environment configured on target"

    c_info "Creating systemd service on target..."
    local svc_file="/tmp/entelecheia-scepter.service"
    cat > "$svc_file" <<SVC
[Unit]
...
SVC
    scp_to_target "$svc_file" "${DEPLOY_USER}@${TARGET_IP}:${remote_install}/scepter.service"
    rm -f "$svc_file"
    ssh_exec "sudo mkdir -p ${LOG_DIR} && sudo chown ${DEPLOY_USER} ${LOG_DIR} && \
        sudo mv ${remote_install}/scepter.service /etc/systemd/system/ && \
        sudo systemctl daemon-reload && \
        sudo systemctl enable scepter && \
        sudo systemctl restart scepter 2>&1" || c_warn "systemd setup failed — try manual install"
    sleep 3
    if ssh_exec "systemctl is-active scepter" 2>/dev/null | grep -q active; then
        c_ok "scepter service running on target"
    else
        c_warn "scepter may not have started — check: ssh ${DEPLOY_USER}@${TARGET_IP} systemctl status scepter"
    fi

    c_info "Opening port ${SCEPTER_PORT} on target..."
    ssh_exec "sudo ufw allow ${SCEPTER_PORT}/tcp 2>/dev/null || \
              sudo firewall-cmd --add-port=${SCEPTER_PORT}/tcp --permanent 2>/dev/null && \
              sudo firewall-cmd --reload 2>/dev/null || true"
    c_ok "Firewall rule attempted for port ${SCEPTER_PORT}"
}
...
ensure_entelecheia_env() {
    ...
    if [[ -z "$api_key" ]]; then
        api_key="sk-your-key-here"
        c_warn "No API key found in opencode config — using placeholder"
    fi
    if [[ -f "$dir/.env.example.minimal" ]]; then
        sed -e "s|LLM_API_KEY=sk-your-key-here|LLM_API_KEY=${api_key}|" \
            -e "s|LLM_BASE_URL=https://api.openai.com/v1|LLM_BASE_URL=https://api.deepseek.com/v1|" \
            -e "s|LLM_MODEL=gpt-4o|LLM_MODEL=deepseek-chat|" \
            "$dir/.env.example.minimal" > "$dir/.env"
        c_ok ".env created from .env.example.minimal with auto-detected API key"
    elif [[ -f "$dir/.env.example" ]]; then
        sed -e "s|LLM_API_KEY=sk-your-key-here|LLM_API_KEY=${api_key}|" \
            "$dir/.env.example" > "$dir/.env"
        c_ok ".env created from .env.example with auto-detected API key"
    else
        cat > "$dir/.env" <<ENVEOF
LLM_API_KEY=${api_key}
LLM_BASE_URL=https://api.deepseek.com/v1
...
RUST_LOG=info
ENVEOF
        c_ok ".env created (minimal, auto-detected API key)"
    fi
    c_info "API key and DeepSeek provider configured in: $dir/.env"
}
...
main() {
    ...
    printf '%.0s─' {1..64}; echo
    c_info "OS: $(uname -sr)"
    [[ -n "$TARGET_IP" ]] && c_info "Target: ${DEPLOY_USER}@${TARGET_IP}"
    [[ -n "$OFFLINE_DIR" ]] && c_info "Offline bundle: ${OFFLINE_DIR}"
    ...
    # Try offline binaries first
    copy_binaries_from_offline || true

    # Phase 3
    ensure_entelecheia_env
    local scepter_ok="fail" evernight_ok="fail"
    if [[ -x "$INSTALL_DIR/entelecheia" ]] || [[ -x "$INSTALL_DIR/scepter" ]]; then
        c_ok "scepter already available (offline)"
        scepter_ok="ok"
    elif build_entelecheia; then scepter_ok="ok"; fi
    if [[ -x "$INSTALL_DIR/evernight" ]]; then
        c_ok "evernight already available (offline)"
        evernight_ok="ok"
    elif build_evernight;  then evernight_ok="ok"; fi
    start_postgres

    # Phase 4
    local scriptum_ok="fail" shittim_ok="fail"
    SHITTIM_EXE=""
    if [[ -x "$INSTALL_DIR/scriptum" ]]; then
        c_ok "scriptum already available (offline)"
        scriptum_ok="ok"
    elif build_scriptum; then scriptum_ok="ok"; fi
    if [[ -x "$INSTALL_DIR/chest-cli" ]]; then
        c_ok "shittim-chest already available (offline)"
        shittim_ok="ok"
    elif build_shittim_chest; then shittim_ok="ok"; fi

    # Phase 5
    install_shortcuts

    # Phase 6: Deploy to remote target
    if [[ -n "$TARGET_IP" && "$SKIP_DEPLOY" -eq 0 ]]; then
        deploy_to_target "$scepter_ok" "$evernight_ok" "$scriptum_ok" "$shittim_ok"
    fi

    # Phase 7: Start locally
