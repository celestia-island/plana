...
.PARAMETER Offline          Path to pre-downloaded offline bundle directory.
                           If set, skips all network downloads and uses
                           local rootfs tarball + container images.
                           The directory must contain:
                             celestia-rootfs-*.tar.gz
                             celestia-images-*.tar.gz (optional, skips podman build)
                           Generated by arona/scripts/rootfs/build-*.sh.
...
param(
    ...
    [switch]$NoMirror,   [string]$Mirror,    [string]$Offline, [switch]$Dev, [switch]$Quick
)
...
$script:WSLDistro    = ""
# Offline bundle directory — pre-downloaded rootfs + container images.
$script:OfflineDir   = if ($Offline -and (Test-Path $Offline)) { (Resolve-Path $Offline).Path } else { $null }
$script:OfflineRootfs = if ($script:OfflineDir) { Get-ChildItem -Path $script:OfflineDir -Filter "celestia-rootfs-*.tar.gz" -ErrorAction SilentlyContinue | Select-Object -First 1 } else { $null }
$script:OfflineImages = if ($script:OfflineDir) { Get-ChildItem -Path $script:OfflineDir -Filter "celestia-images-*.tar.gz"  -ErrorAction SilentlyContinue | Select-Object -First 1 } else { $null }
...
function New-CelestiaWslInstance {
    ...
    # Prefer offline rootfs tarball if available.
    if ($script:OfflineRootfs -and (Test-Path $script:OfflineRootfs.FullName)) {
        $rootfsFile = $script:OfflineRootfs.FullName
        Write-Info "Using offline rootfs: $(Split-Path $rootfsFile -Leaf) ($([math]::Round((Get-Item $rootfsFile).Length / 1MB, 1)) MB)"
    } else {
    ...
    & wsl --import $name $installLoc $rootfsFile --version 2 2>&1 | Out-Null
    if ($LASTEXITCODE -ne 0) {
        Write-Err "wsl --import failed for $name"
        Remove-Item $rootfsFile -Force -ErrorAction SilentlyContinue
        exit 1
    }
    # Only clean up temp download; preserve offline bundle files.
    if (-not $script:OfflineRootfs) {
        Remove-Item $rootfsFile -Force -ErrorAction SilentlyContinue
    }
...
function Load-OfflineImagesInWSL {
    param([string]$WslSourcePath)
    if (-not $script:OfflineImages) { return $false }
    Write-Step "Loading pre-pulled container images from offline bundle"
    $imagesPath = Convert-WinPathToWSL -Path $script:OfflineImages.FullName
    $r = Invoke-WSL -Command "podman load -i '$imagesPath' 2>&1 && echo IMAGES_LOADED || echo IMAGES_FAILED"
    if ($r -match "IMAGES_LOADED") {
        Write-Ok "Pre-pulled images loaded: $(Split-Path $script:OfflineImages.FullName -Leaf)"
        return $true
    }
    Write-Warn "Image load returned non-OK — falling back to network pull."
    return $false
}
...
function Build-EntelecheiaInWSL {
    param([string]$WslSourcePath, [bool]$OfflineImagesLoaded = $false)
    if ($SkipBuild) { Write-Info "Skipping entelecheia build (-SkipBuild)"; return $false }
    if ($OfflineImagesLoaded) {
        Write-Step "Skipping entelecheia build — using offline bundle images"
        Write-Info "The entelecheia:latest image is expected to be in the offline bundle."
        Write-Info "If you need to rebuild from source, omit -Offline and re-run."
        # Verify the image exists after load
        $r = Invoke-WSL -Command "podman image exists entelecheia:latest 2>/dev/null && echo IMAGE_OK || echo IMAGE_MISSING"
        if ($r -match "IMAGE_OK") {
            Write-Ok "entelecheia:latest image confirmed"
            return $true
        }
        Write-Warn "entelecheia:latest not found in offline bundle — building from source."
    }
...
function Main {
    ...
    # Load pre-pulled container images from offline bundle if available
    $offlineImagesLoaded = Load-OfflineImagesInWSL -WslSourcePath $wslSource

    $scepterBuilt   = Build-EntelecheiaInWSL -WslSourcePath $wslSource -OfflineImagesLoaded:$offlineImagesLoaded
