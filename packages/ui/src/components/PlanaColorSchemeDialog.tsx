...
import "./PlanaColorSchemeDialog.scss";
...
/**
 * PCustomTheme — custom color scheme payload, structurally identical to
...
/**
 * PColorSchemeDialog — custom color scheme editor over hikari's theme
 ...
 * `confirm` with a `PCustomTheme` (a hikari `CustomThemePreset`); the
 * caller persists it via `useTheme().addCustomTheme()` and applies it
 * with `setTheme()`.
 */
export const PColorSchemeDialog = defineComponent({
  name: "PlanaColorSchemeDialog",
  props: {
  ...
  emits: {
    ...
    confirm: (_theme: PCustomTheme) => true,
  },
  setup(props, { emit }) {
    const { t } = useI18n();
    ...
    watch(
      ...
      (open) => {
        ...
        themeName.value = t("plana::theme.customThemeName");
    ...
    function handleConfirm() {
      ...
      emit("confirm", {
        id,
        name: themeName.value || t("plana::theme.customThemeName"),
        dark: { ...dark },
        light: { ...light },
      });
    ...
    const footerActions = computed<ModalAction[]>(() => [
      {
        label: t("plana::protocol.decline"),
        variant: "secondary" as const,
        onClick: () => emit("update:modelValue", false),
      },
      {
        label: t("plana::theme.save"),
    ...
    const modeTabs = computed<HkTabItem[]>(() => [
      { key: "dark", label: t("plana::theme.modeDark") },
      { key: "light", label: t("plana::theme.modeLight") },
    ]);
    ...
    return () => (
      <HModal
        ...
        title={t("plana::theme.editScheme")}
        width="36rem"
      ...
      >
        <div class="s-scheme-dialog">
          <HInput
            ...
            label={t("plana::theme.themeName")}
            placeholder={t("plana::theme.customThemeName")}
          />
          <HMorphingTabs
            tag="div"
          ...
          <div class="s-scheme-colors">
            {editableTokens.map((key) => (
              <HColorPicker
                ...
                label={t(`plana::theme.tokens.${key}`)}
