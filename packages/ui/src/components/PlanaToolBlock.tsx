...
export interface PHighlightedLine {
  num: string;
  html: string;
}
...
export function buildHighlightedLines(code: string, language: string): PHighlightedLine[] {
 ...
 */
export function buildJsonTree(value: unknown, maxDepth = 8): PJsonNode | null {
  if (value === null || typeof value !== "object") return null;
  return buildJsonNode(value, null, 0, maxDepth, { n: 0 });
}

function initialExpandedSet(root: PJsonNode): Set<number> {
...
export const PToolBlock = defineComponent({
  ...
  setup(props) {
    ...
    function renderJsonNode(node: PJsonNode) {
    ...
    function renderLongString(node: PJsonNode, indent: number) {
      const isOpen = jsonExpanded.value.has(node.id);
