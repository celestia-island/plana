...
import { useI18n, mergeMessages } from "@celestia-island/hikari";
import { PStatusBar } from "./PlanaStatusBar";
import {
  useConnectionInfo,
  type ConnectionStateInput,
  type PlanaConnectionInfo,
} from "./PlanaConnectionInfo";
...
export interface PBackendStatus {
  label: string;
  state: "ok" | "warn" | "error" | "unknown";
}

const backendStateColor: Record<PBackendStatus["state"], string> = {
 ...
 * useEngineHealth + useConnectionInfo + PStatusBar. Slots:
 * - `left`  — extras after the status tags (e.g. a mock badge)
 * - `right` — extras on the far right (e.g. an engine version tag)
 */
export const PConnectionStatus = defineComponent({
  name: "PlanaConnectionStatus",
  props: {
    ...
    backendStatus: { type: Object as PropType<PBackendStatus | undefined>, default: undefined },
  ...
  setup(props, { slots }) {
    ...
    const mergedInfo = computed<PlanaConnectionInfo>(() => {
            ...
            {bs && (
              <span class="s-status-bar-tag" style={{ cursor: "default" }}>
                ...
                <span class="s-status-bar-tag-value">
                  {t(`plana::statusBar.backend.${bs.state}`, bs.state)}
                </span>
              </span>
            )}
            {slots.left?.()}
