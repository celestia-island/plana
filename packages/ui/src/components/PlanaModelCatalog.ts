/**
 * Model-spec catalog for the PModelTag hover card.
 *
 * The catalog is a plain `Record<modelId, ModelMeta>` map. plana ships a
 * small built-in set of plausible specs as a read-only fallback; services
 * (arona, shittim-chest) provide their own live data via `meta` props or
 * `registerModelCatalog`.
 */

export interface PModelPricing {
...
export interface PModelMeta {
  /** Max input context window (tokens) */
  contextWindow?: number;
  /** Max output tokens */
  maxOutput?: number;
  pricing?: PModelPricing;
...
export type PModelCatalog = Record<string, PModelMeta>;

const BUILTIN_CATALOG: PModelCatalog = {
  "deepseek-v4-pro": {
 ...
 */
let extraCatalog: PModelCatalog = {};

export function registerModelCatalog(entries: PModelCatalog): void {
 ...
 */
export function getModelMeta(model: string, catalog?: PModelCatalog): PModelMeta | undefined {
