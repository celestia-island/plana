...
pub trait SkillInvoker: Send + Sync {
    ...
    fn get_skills(&self) -> Vec<plana_state_sync::SkillInfo>;
}
...
pub struct Skill {
    ...
    pub next_action: Vec<plana_prompt::prompt_loader::StepAction>,
...
impl Skill {
    ...
    /// ```
    pub fn parse_steps(content: &str) -> Vec<plana_prompt::prompt_loader::StepAction> {
        let mut next_action = Vec::new();

        if let Some(parts) = plana_prompt::front_matter::extract_front_matter(content)
            ...
            for item in ns {
                if let Some(table) = item.as_table() {
                    ...
                    if !agent.is_empty() && !name.is_empty() {
                        next_action.push(plana_prompt::prompt_loader::StepAction {
    ...
    pub fn parse_must_touch_next_action(content: &str) -> bool {
        plana_prompt::front_matter::extract_front_matter(content)
    ...
    pub fn to_info(&self) -> plana_state_sync::SkillInfo {
        let location = self
            .metadata
            .features
            .location
            .unwrap_or(plana_state_sync::tools::SkillLocation::Scepter);
        plana_state_sync::SkillInfo {
...
impl SkillRegistry {
    ...
    /// Get all Skills
    pub async fn list_all(&self) -> Vec<plana_state_sync::SkillInfo> {
        let skills = self.skills.read().await;
        skills.values().map(|skill| skill.to_info()).collect()
    }

    /// Get Skills for specified Agent
    pub async fn list_by_agent(&self, agent_type: Agent) -> Vec<plana_state_sync::SkillInfo> {
        let skills = self.skills.read().await;
        skills
            .values()
