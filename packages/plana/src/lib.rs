//! Typed JSON-RPC 2.0 protocol toolkit.
//!
//! `plana` is a general-purpose protocol library. It provides shared wire
//! types, JSON-RPC 2.0 machinery, and server-side session management, split
//! into feature-gated modules so consumers pull in only what they need:
//!
//! - **`types`** (default) — shared wire types: the JSON-RPC envelope
...
//!   generation support.
//! - **`jsonrpc`** — the JSON-RPC 2.0 layer: request/response/notification
//!   types, a method registry ([`jsonrpc::rpc_router::RpcMethodMap`]),
...
//! - **`rpc-server`** — server-side session management
//!   ([`rpc_server::SessionManager`]), SSE event streaming, and request
//!   network/geolocation detection ([`rpc_server::detect_network`]).
//!
//! The `types` module follows a strict rule: a type belongs there only when
//! it is defined in this crate as the canonical source of truth and consumed
//! on both sides of a wire protocol. Anything else stays out.
...
//   jsonrpc/  — JSON-RPC 2.0 types, method registry, Unix socket transports
//   rpc_server/ — session management, SSE, network detection
...
// at the crate root (`plana::TypeName`).
pub mod engine;
pub mod enums;
...
// `plana::TuiAgentInfo`, `plana::HandshakeAckParams`, etc. all still resolve.
//
// `jsonrpc` is re-exported *as a module* (not globbed) so its deep path
// `plana::jsonrpc::*` keeps working for the many consumers that use it.
// ═══════════════════════════════════════════════════════════════

// protocol/ — transport core
pub use protocol::base_messages::*;
pub use protocol::handshake::*;
// The protocol envelope copy is re-exported at the crate root (`plana::jsonrpc`)
// only while the feature-gated `jsonrpc` module is disabled; when the `jsonrpc`
// feature is on, `plana::jsonrpc` is the JSON-RPC module itself.
